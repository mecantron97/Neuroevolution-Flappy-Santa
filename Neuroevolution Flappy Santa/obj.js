...
class Obj {
  ...
  think(pipes) {
    ...
    // Im functie de cel mai apropiat obstacol creaza
    // inputurile retelei neuronale
    if (closest != null) {
      // Inputuri
      let inputs = [];
      // Coordonata x a celui mai apropiat obiect
      inputs[0] = map(closest.x, this.x, width, 0, 1);
      // Coordonata y a deschiderii obstacolului superior
      inputs[1] = map(closest.top, 0, height, 0, 1);
      // Coordonata y a deschiderii obstacolului inferior
      inputs[2] = map(closest.bottom, 0, height, 0, 1);
      // Coordonata y a obiectului
      inputs[3] = map(this.y, 0, height, 0, 1);
      // Viteza de avans a obiectului
